...
import { db, projectsTable, projectCollaboratorsTable, usersTable } from "@workspace/db";
...
router.post("/projects", authMiddleware, async (req, res): Promise<void> => {
  const authUser = (req as any).user;
  const { title, description, priority, category, type, ownerId, processOwnerId, startDate, endDate, reviewFrequency } = req.body;
...
router.patch("/projects/:id", authMiddleware, async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  const { title, description, status, priority, ownerId, processOwnerId, startDate, endDate } = req.body;
